...
const OrderReportComponent = () => {
  const { OrderReportData, isLoading, errorMessage, router, purity } = useOrderReport();
  ...
  const handleDataRendering = () => {
    ...
    if (OrderReportData?.length > 0) {
      return <OrderReportTableMaster tableBodyData={OrderReportData} title={title} purity={purity}/>;
    }
