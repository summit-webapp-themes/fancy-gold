...
const SizeQtyTable = ({ data, onQtyChange, onDelete }: any) => {
  ...
  return (
    <>
      <div className={`row ${styles.font_12} text-center flex-nowrap mx-0`}>
        <div className="col-md-2 col-3 border border-start-0 p-1">Colour</div>
        ...
        <div className="col-md-1 col-1 border border-end-0 p-1"></div>
      </div>
      {data?.order?.length > 0 &&
        data?.order?.map((item: any, index: number) => (
          <div className={`row ${styles.font_12} text-center flex-nowrap mx-0`} key={index}>
            <div className="col-md-2 col-3 border border-start-0 p-1">{item?.colour}</div>
            ...
            <div className="col-md-1 col-1 border border-end-0 p-1 px-0 d-flex justify-content-center align-items-center cursor-pointer">
              <RxCross2 onClick={() => onDelete(data.item_code, item.size)} />
            </div>
          </div>
        ))}
      <div className={`row ${styles.font_12} text-center mx-0`}>
        <div className="col-5 border border-start-0 py-2">Total weight</div>
        <div className="col-7 border border-end-0 py-2 text-end">{Number(data?.total_weight).toFixed(3)}gm</div>
      </div>
    </>
  );
};
