...
const CartListing = () => {
  ...
  useEffect(() => {
    if (
      ...
      cartListingItems?.categories[0]?.orders?.length > 0 &&
      cartListingItems?.categories[0]?.orders[0]?.order?.length > 0
    ) {
      const colorOfItems: string = cartListingItems.categories[0].orders[0].order[0]?.colour;
      if (colorOfItems) {
